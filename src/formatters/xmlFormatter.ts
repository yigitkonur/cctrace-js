...
import { cleanTextForXml } from '../utils';
...
export class XmlFormatter {
  ...
  static formatMessage(messageData: MessageData, parentElement: any): void {
    ...
    // Add attributes
    if (messageData.uuid) msgElem.att('uuid', messageData.uuid);
    if (messageData.parentUuid) msgElem.att('parent-uuid', messageData.parentUuid);
    if (messageData.timestamp) msgElem.att('timestamp', messageData.timestamp);
    ...
    if (messageData.message) {
      ...
      if (msg.content) {
        ...
        if (typeof msg.content === 'string') {
          contentElem.txt(cleanTextForXml(msg.content));
          ...
          for (const content of msg.content) {
            if (typeof content === 'object' && content !== null) {
              ...
              if (contentType === 'text') {
                ...
                inputElem.txt(cleanTextForXml(JSON.stringify(content.input || {}, null, 2)));
                ...
                if (typeof resultContent === 'string') {
                  resultElem.txt(cleanTextForXml(resultContent));
                } else {
                  resultElem.txt(cleanTextForXml(String(resultContent)));
                }
              }
            }
          }
  ...
  static formatSession(messages: MessageData[], metadata: SessionMetadata): string {
    ...
    if (metadata.sessionId) metaElem.ele('session-id').txt(metadata.sessionId);
    ...
    if (metadata.projectDir) metaElem.ele('working-directory').txt(metadata.projectDir);
    if (metadata.startTime) metaElem.ele('start-time').txt(metadata.startTime);
    if (metadata.endTime) metaElem.ele('end-time').txt(metadata.endTime);
